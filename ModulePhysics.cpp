...
update_status ModulePhysics::PostUpdate()
{
	...
	for(b2Body* b = world->GetBodyList(); b; b = b->GetNext())
	{
		for(b2Fixture* f = b->GetFixtureList(); f; f = f->GetNext())
		{
			...
			if (App->input->GetMouseButton(SDL_BUTTON_LEFT) == KEY_DOWN) {
				v.Set(PIXEL_TO_METERS(App->input->GetMouseX()), PIXEL_TO_METERS(App->input->GetMouseY()));
				if (s->TestPoint(t, v)) {
					targetbody = b;
					b2MouseJointDef def;
					def.bodyA = ground;
	...
	if (mouse_joint != nullptr && App->input->GetMouseButton(SDL_BUTTON_LEFT) == KEY_REPEAT && targetbody != nullptr) {
		v.Set(PIXEL_TO_METERS(App->input->GetMouseX()), PIXEL_TO_METERS(App->input->GetMouseY()));
		mouse_joint->SetTarget(v);
		App->renderer->DrawLine(METERS_TO_PIXELS(targetbody->GetPosition().x), METERS_TO_PIXELS(targetbody->GetPosition().y), METERS_TO_PIXELS(v.x), METERS_TO_PIXELS(v.y), 255, 0, 0);
	}
...
void ModulePhysics::BeginContact(b2Contact* contact)
{
	...
	while (c != NULL) {
		...
		if (physA == App->scene_intro->leftFlipper && physB == c->data) {
			...
			App->scene_intro->extra = 0;
			App->audio->PlayFx(App->scene_intro->bonus_fx);
		}

		if (physA == App->scene_intro->rightFlipper && physB == c->data) {
			App->scene_intro->score += 200;
			App->scene_intro->extra = 0;
			App->audio->PlayFx(App->scene_intro->bonus_fx);
		}

		if (physA == App->scene_intro->sensor && physB == c->data) {
